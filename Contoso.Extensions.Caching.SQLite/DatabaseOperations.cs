...
namespace Contoso.Extensions.Caching.SQLite
{
    ...
    {
        const int DuplicateKeyErrorId = 2627;
        protected const string GetTableSchemaErrorText = "Could not retrieve information of table with schema '{0}' and name '{1}'. Make sure you have the table setup and try again. Connection string: {2}";
        ...
        public DatabaseOperations(string connectionString, string schemaName, string tableName, ISystemClock systemClock)
        {
            ...
            SqlQueries = new SQLiteQueries(schemaName, tableName);
        }
        ...
        public virtual void SetCacheItem(string key, byte[] value, DistributedCacheEntryOptions options)
        {
            ...
            using (var connection = new SQLiteConnection(ConnectionString))
            using (var upsertCommand = new SQLiteCommand(SqlQueries.SetCacheItem, connection))
            {
